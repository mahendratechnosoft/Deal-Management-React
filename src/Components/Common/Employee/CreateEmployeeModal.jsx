...
function CreateEmployeeModal({ onClose, onSuccess }) {
  ...
  const loadRoles = async () => {
    ...
    try {
      ...
      const options = response.data.map((role) => ({
        ...
        donorAccess: role.donorAccess,
        donorViewAll: role.donorViewAll,
        donorCreate: role.donorCreate,
        donorDelete: role.donorDelete,
        donorEdit: role.donorEdit,

        itemAccess: role.itemAccess,
        itemViewAll: role.itemViewAll,
        ...
        complianceAccess: role.complianceAccess,
        complianceViewAll: role.complianceViewAll,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...formData,
        moduleAccess: moduleAccess, // Send moduleAccess as a separate object
      };
      ...
      if (error.response?.data?.message) {
        toast.error(
          `Failed to create employee: ${error.response.data.message}`
        );
      } else if (
      ...
      <div className="flex justify-end mb-2">
        <div className="flex gap-2">
          <button
            type="button"
            onClick={onClearAll}
            className="px-2 py-1 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors duration-200"
          >
            Clear All
          </button>
          {/* <button
            type="button"
            onClick={onSelectAll}
            className="px-2 py-1 text-sm font-medium text-white bg-green-600 border border-transparent rounded-lg hover:bg-green-700 transition-colors duration-200"
          >
            Select All Access
          </button> */}
        </div>
            ...
            <div className="flex items-center gap-2">
              {activeTab === "permissions" && (
                <button
                  type="button"
                  onClick={() => setActiveTab("personal")}
                  className="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 bg-white hover:bg-gray-50 transition-all duration-200 font-medium text-sm"
                >
                  Previous
                </button>
              )}
              ...
              {activeTab === "permissions" && (
                <button
                  ...
                  disabled={loading}
                  className="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-all duration-200 font-medium flex items-center gap-2 text-sm"
                >
                  {loading ? (
                    <>
