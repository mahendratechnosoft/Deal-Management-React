...
const SendProposalEmailModal = ({
  ...
  const fetchProposalDataForPdf = useCallback(async () => {
    ...
    try {
      ...
      return proposalDataToSend;
    } catch (error) {
      console.error("Error fetching proposal data for PDF:", error);
      toast.error("Failed to fetch proposal data for PDF generation");
      return null;
    }
  }, [proposalId, role]);

  // Generate PDF as base64
  const generatePdfBase64 = useCallback(async (proposalData) => {
    ...
    try {
      ...
      // Create PDF document
      const doc = <ProposalPDF data={proposalData} />;
  ...
  useEffect(() => {
    const generateProposalPdf = async () => {
      if (!isOpen || !proposalId || !formData.attachPdf) return;

      try {
        // Fetch proposal data
        const data = await fetchProposalDataForPdf();
        if (!data) {
          setPdfBase64("");
          return;
        }
    ...
    proposalId,
    formData.attachPdf,
    fetchProposalDataForPdf,
    generatePdfBase64,
  ]);

  // Fetch templates for PROPOSAL_SEND trigger
  useEffect(() => {
    const fetchTemplates = async () => {
      ...
      try {
        const response = await axiosInstance.get(
          "getEmailTemplates/SALES/PROPOSAL_SEND"
        );
  ...
  useEffect(() => {
    const fetchPlaceholders = async () => {
      ...
      try {
        const response = await axiosInstance.get(
          "getTampletePlaceholders/PROPOSAL_SEND"
        );
  ...
  // Handle checkbox change - SIMPLIFIED VERSION
  ...
  const handleSendEmail = async (e) => {
    ...
    try {
      ...
      const response = await axiosInstance.post("sendProposalEmail", payload);
          ...
          <div className="bg-gradient-to-r from-blue-600 to-indigo-700 text-white p-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-xl font-bold">Send Proposal via Email</h2>
          ...
          >
            <form onSubmit={handleSendEmail}>
              <div className="space-y-6">
                ...
                {/* SIMPLIFIED PDF Attachment Checkbox */}
                <div className="space-y-2">
