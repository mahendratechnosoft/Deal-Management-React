...
import ProformaPDF from "../Proforma/ProformaPDF"; // Reusing the same PDF component
import { useLayout } from "../../Layout/useLayout";

const SendInvoiceEmailModal = ({
  isOpen,
  onClose,
  invoiceId,
  invoiceNumber,
  ...
  const fetchInvoiceDataForPdf = useCallback(async () => {
    if (!invoiceId) return null;

    try {
      let adminInformation = null;
      const response = await axiosInstance.get(
        `getProformaInvoiceById/${invoiceId}`
      );
      ...
      return invoiceDataToSend;
    } catch (error) {
      console.error("Error fetching invoice data for PDF:", error);
      toast.error("Failed to fetch invoice data for PDF generation");
      return null;
    }
  }, [invoiceId, role]);

  // Generate PDF as base64
  const generatePdfBase64 = useCallback(async (pdfData) => {
    if (!pdfData || !pdfData.invoiceData || !pdfData.adminInformation) {
      console.log("Missing required data for PDF generation:", pdfData);
      return "";
    }
    ...
    try {
      ...
      // Create PDF document - Use ProformaPDF component with isInvoice=true
      const doc = (
        <ProformaPDF
          invoiceData={pdfData.invoiceData}
          adminInformation={pdfData.adminInformation}
          isInvoice={true}
        />
      );
  ...
  useEffect(() => {
    const generateInvoicePdf = async () => {
      if (!isOpen || !invoiceId || !formData.attachPdf) return;

      try {
        // Fetch invoice data
        const data = await fetchInvoiceDataForPdf();
        if (!data) {
          setPdfBase64("");
          return;
        }
    ...
    invoiceId,
    formData.attachPdf,
    fetchInvoiceDataForPdf,
    generatePdfBase64,
  ]);

  // Fetch templates for INVOICE_SEND trigger
  useEffect(() => {
    const fetchTemplates = async () => {
      ...
      try {
        const response = await axiosInstance.get(
          "getEmailTemplates/SALES/INVOICE_SEND"
        );
  ...
  // Fetch placeholders for INVOICE_SEND
  useEffect(() => {
    const fetchPlaceholders = async () => {
      ...
      try {
        const response = await axiosInstance.get(
          "getTampletePlaceholders/INVOICE_SEND"
        );
  ...
  const handleSendEmail = async (e) => {
    ...
    try {
      ...
      // Use the invoice-specific API endpoint
      const response = await axiosInstance.post("sendInoviceEmail", payload);
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    placeholder="e.g., Invoice: {{invoiceNumber}}"
                ...
                <div className="space-y-2">
                  ...
                  <RichTextEditor
                    ...
                    placeholder="Enter your email content here. Use placeholders like {{invoiceNumber}}, {{clientName}}, {{totalAmount}}, etc."
                    height="250px"
                    toolbarConfig="email"
                    label=""
