...
const modeRef = useRef(null);
    ...
    const fetchInvoices = async () => {
        try {
            ...
            if (response.data.statusCode === "OK" && response.data.body) {
                ...
                console.log("Filtered invoices:", validInvoices);
    ...
    const handleInvoiceChange = (selectedOption) => {
        if (selectedOption) {
            const invoice = invoices.find(i => i.proformaInvoiceId === selectedOption.value);
            if (invoice) {
 ...
 const validateForm = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await axiosInstance.post("/admin/createPayment", submitData);
            ...
            if (error.response?.data?.message) {
                toast.error(
                    `Failed to create payment: ${error.response.data.message}`
                );
            } else {
    ...
    const invoiceOptions = invoices.map(invoice => ({
        ...
        label: `${invoice.proformaInvoiceNo} - ${invoice.companyName} - Total: ₹${(invoice.totalAmount || 0).toLocaleString()}`
    }));
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Select
                                        ...
                                        value={invoiceOptions.find(option => option.value === formData.proformaInvoiceId)}
                                        onChange={handleInvoiceChange}
                                        options={invoiceOptions}
                                        placeholder={fetchLoading ? "Loading invoices..." : "Select proforma invoice"}
                                        isSearchable
                                    ...
                                    {!fetchLoading && invoiceOptions.length === 0 && (
                                        <p className="text-yellow-600 text-xs flex items-center gap-1">
                                            ...
                                            No valid invoices found. Please check if invoices have invoice numbers.
                                        </p>
                                    )}
                                </div>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <input
                                        ...
                                        className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 ${errors.transactionId ? "border-red-500" : "border-gray-300"
                                            }`}
                                ...
                                <div className="space-y-2">
                                    ...
                                    <input
                                        ...
                                        onChange={handleChange}
                                         ref={amountRef}
                                        className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 ${errors.amount ? "border-red-500" : "border-gray-300"
                                            }`}
                                    ...
                                    {selectedInvoice && (
                                        <div className="text-xs text-gray-700 space-y-1 mt-1">
                                            <p>Total Amount: <span className="font-semibold text-gray-900">₹{parseFloat(formData.totalAmount || 0).toLocaleString()}</span></p>
                                            <p>Paid Amount: <span className="font-semibold text-gray-900">₹{parseFloat(formData.paidAmount || 0).toLocaleString()}</span></p>
                                            <p>Remaining Amount: <span className="font-semibold text-gray-900">₹{remainingAmount.toLocaleString()}</span></p>
                                        </div>
                                    )}
                                ...
                                <div className="space-y-2">
                                    ...
                                    <input
                                        ...
                                        onChange={handleChange}
                                            ref={dateRef}
                                        className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 ${errors.paymentDate ? "border-red-500" : "border-gray-300"
                                            }`}
                                    />
                                    {errors.paymentDate && (
                                        <p className="text-red-500 text-xs flex items-center gap-1">
