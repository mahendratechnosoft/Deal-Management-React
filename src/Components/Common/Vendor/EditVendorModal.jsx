...
import { showErrorAlert } from "../../BaseComponet/alertUtils";
...
const [attachmentsLoading, setAttachmentsLoading] = useState(false);
  ...
  useEffect(() => {
    if (activeTab === "attachments" && formData.vendorId && canEdit) {
...
const fetchVendorAttachments = async () => {
  ...
  // Handle remove attachment with confirmation
  const handleRemoveAttachment = async (id, fileName, isExisting = false) => {
    try {
      console.log("Attempting to delete attachment with ID:", id);

      // Check if ID is valid (not a temporary ID)
      if (!id || id.startsWith("temp-")) {
        console.error("Invalid or temporary ID for deletion:", id);
        toast.error("Cannot delete attachment with invalid ID");

        // Still remove from UI if it's a temporary attachment
      ...
      // Call delete API with the correct ID
      ...
      if (response.status === 200 || response.status === 204) {
        ...
        // Show success alert
        showSuccessAlert("Attachment deleted successfully", "Deleted!");
      ...
      // Optional: Show more details in console
      if (error.response) {
