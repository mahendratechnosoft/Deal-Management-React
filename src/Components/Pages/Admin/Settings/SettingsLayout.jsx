...
import { Outlet, NavLink, useNavigate, useLocation } from "react-router-dom"; // Import useLocation
...
const SettingsLayout = () => {
  ...
  const location = useLocation(); // Needed for icon color logic
  ...
  // UPDATED: Styling function for the LIGHT settings sidebar
  const getNavLinkClass = ({ isActive }) => {
    ...
    if (isActive) {
      // Active: Keep the vibrant gradient to match the main sidebar's active state
      return `${baseClasses} bg-gradient-to-r from-blue-500 to-cyan-500 shadow transform scale-105 text-white font-medium`;
    }
    // Inactive: Use a standard light-theme hover
